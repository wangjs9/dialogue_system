...
if TYPE_CHECKING:
    from transformers import PretrainedConfig, PreTrainedModel, PeftConfig
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            if finetuning_args.simulator_model and finetuning_args.simulator_model_type == "lora":
                model: "LoraModel" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
                model.load_adapter(finetuning_args.simulator_model, adapter_name='simulator')
            else:
                model: "LoraModel" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
